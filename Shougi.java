...
class Teban{
    ...
    int getKomaType(){ return komaType; }
...
class Koma{
	...
	if(name==fu|| name==kyo|| name==kei|| name==silver|| name==gold || name==hi || name==kaku || name==king || name==gyoku || name==tokin || name==narikyo || name==narikei || name==narigin || name==ryuou || name==ryuma ||name==none)
	    obverse=name;
	else
...
class Board{
	...
	else if(koma[y][x].getObverse()==Koma.gold || koma[y][x].getObverse()==Koma.tokin || koma[y][x].getObverse()==Koma.narikyo || koma[y][x].getObverse()==Koma.narikei || koma[y][x].getObverse()==Koma.narigin){
	    for(int i=0; i<7; i++){
		if(i==5)
		    continue;
		cx=x+d[i].x;
	...
	public void mouseClicked(MouseEvent me){
		...
		if(board.enemyKoma[gp3.x].getObverse()!=Koma.none){
			...
			for(int j=0; j<9; j++){
			    ...
			    if(board.koma[i][j].getObverse()==Koma.fu && board.teban[i][j].getKomaType()==Teban.enemy && enemyKoma.get(gp3.x)==Koma.fu)
				stack_j.add(j);
		...
		if(board.myKoma[gp3.x].getObverse()!=Koma.none){
			...
			for(int j=0; j<9; j++){
			    if(board.koma[i][j].getObverse()==Koma.none)
				board.move[i][j].setPlace(Move.can);
			    if(board.koma[i][j].getObverse()==Koma.fu && board.teban[i][j].getKomaType()==Teban.you && myKoma.get(gp3.x)==Koma.fu)
				stack_j.add(j);
